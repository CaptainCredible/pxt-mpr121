// mpr121 touch blocks modified from makerbit
...
namespace makerbit {
    ...
    function getSensorIndexFromSensorBitField(touchSensorBit: TouchSensor) {
        if (touchSensorBit === TouchSensor.Any) return TouchSensor.Any;
        for (let i = 0; i < 12; i++) {
            if ((touchSensorBit & (1 << i)) !== 0) return i; // T0..T11 -> 0..11
        }
        return 0; // none (won't be used when eventValue==0)
    }

    function getTouchSensorFromIndex(index: number): TouchSensor {
        if (0 <= index && index < 12) {
            return 1 << index; // 0..11 -> T0..T11
